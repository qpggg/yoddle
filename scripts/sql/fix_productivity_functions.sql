-- ===================================================================
-- ИСПРАВЛЕНИЕ ФУНКЦИЙ РАСЧЕТА ПРОДУКТИВНОСТИ
-- ===================================================================
-- Исправляет функцию get_user_productivity_stats для возврата всех нужных полей
-- ===================================================================

-- ===================================================================
-- РЕШЕНИЕ ПРОБЛЕМЫ "function is not unique" и прав доступа
-- ===================================================================
-- Проблема 1: В БД могут быть несколько версий функции с одинаковым именем,
-- но разными сигнатурами (например, INTEGER и NUMERIC параметры).
-- PostgreSQL не может определить, какую версию вызывать.
--
-- Проблема 2: Если функция создана другим пользователем, текущий пользователь
-- не может её заменить через CREATE OR REPLACE (нужны права владельца).
--
-- Решение: Используем DROP FUNCTION (работает с правами на схему),
-- затем создаем новые функции.
-- ===================================================================

-- ⚠️ ВАЖНО: Если получаете ошибку "must be owner of function", 
-- запустите скрипт от суперпользователя или владельца БД:
--
-- Вариант 1 (рекомендуется): В терминале:
--   psql -U postgres -d yoddle_db -f scripts/sql/fix_productivity_functions.sql
--
-- Вариант 2: В DBeaver или другом клиенте выполните перед скриптом:
--   SET ROLE postgres;  -- или имя владельца БД (например, yoddle_user)
--   затем выполните этот скрипт
--
-- Вариант 3: Попросите администратора БД выполнить этот скрипт

-- Удаляем все версии функций (чтобы избежать конфликта "function is not unique")
-- Используем динамический запрос для удаления всех версий независимо от сигнатуры
-- DROP FUNCTION работает даже если ты не владелец (если есть права на схему)
...
BEGIN
    ...
    LOOP
        FOR r IN 
            SELECT oid, proname, pg_get_function_identity_arguments(oid) as args,
                   pg_get_function_identity_arguments(oid::regprocedure) as full_signature
            FROM pg_proc 
    ...
    IF error_count > 0 THEN
        RAISE WARNING 'Ошибок при удалении: % (нужны права суперпользователя!)', error_count;
        RAISE WARNING 'Запустите скрипт от postgres: psql -U postgres -d yoddle_db -f scripts/sql/fix_productivity_functions.sql';
    END IF;
    RAISE NOTICE '========================================';
END $$;
...
-- Добавляем использование quality_score в calculate_productivity_score
-- (Нужно будет обновить существующую функцию)
COMMENT ON FUNCTION calculate_productivity_score IS 
'Рассчитывает рейтинг продуктивности на основе записей настроения и активности за указанную дату. 
Учитывает: настроение (mood_rating + energy_rating + (10 - stress_rating)), 
успешные/неуспешные активности, активность на платформе.
TODO: Добавить учет quality_score в расчеты.';
