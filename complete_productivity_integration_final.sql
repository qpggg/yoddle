-- =====================================================
-- ПОЛНАЯ ИНТЕГРАЦИЯ ПРОДУКТИВНОСТИ С БД (ФИНАЛЬНАЯ ВЕРСИЯ)
-- =====================================================
...
-- Включает расширение activity_types и правильную логику подсчета

-- =====================================================
-- 1. РАСШИРЕНИЕ ACTIVITY_TYPES ДЛЯ ОТЧЕТОВ
-- =====================================================

-- Добавляем новые типы для отчетов (если не существуют)
INSERT INTO activity_types (action_code, name, description, xp_reward, icon, category) 
VALUES 
    ('daily_mood_report', 'Ежедневный отчет настроения', 'Отчет настроения, энергии и стресса за день', 25, 'heart', 'productivity'),
    ('daily_activity_report', 'Отчет активности', 'Отчет о выполненных задачах и активностях за день', 30, 'check-square', 'productivity')
...
BEGIN
    ...
    -- Расчет коэффициента активности на платформе (ВСЕ действия за день, включая отчеты)
