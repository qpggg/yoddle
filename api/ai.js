...
// POST /api/ai/generate-telegram-recommendations - Генерация персональных рекомендаций для Telegram бота
router.post('/generate-telegram-recommendations', async (req, res) => {
  try {
    const { mood, task, role, userId } = req.body;
    
    if (!mood || !task || !role) {
      return res.status(400).json({
        success: false,
        error: 'Необходимы параметры: mood, task, role'
      });
    }

    console.log(`🤖 Генерация рекомендаций для Telegram: mood=${mood}, task=${task}, role=${role}`);
...
Используй эмоджи. Будь поддерживающим и конкретным. Упомяни конкретные модули платформы (льготы, геймификация, ИИ-аналитика).`;
    ...
    const advice = cleanClaudeOutput(message.content[0].text);

    // Если передан userId, сохраняем сигнал в БД
    if (userId) {
      try {
        // Для Telegram пользователей используем фиксированный user_id = 1
        // (так как таблица enter только для пользователей сайта)
        const dbUserId = userId > 1000000 ? 1 : userId;
        
        const signalQuery = `
          INSERT INTO ai_signals (
            user_id, type, data, timestamp, 
            mood_rating, stress_rating, notes
          )
        ...
        const signalData = {
          ...
          role: role,
          source: 'telegram_bot',
          telegram_id: userId > 1000000 ? userId : null, // Сохраняем telegram_id если это Telegram пользователь
          timestamp: new Date()
        };
        ...
        console.error('⚠️ Ошибка сохранения в БД (продолжаем):', dbError.message);
      }
    }
    ...
    console.error('❌ Generate telegram recommendations error:', error);
    res.status(500).json({
      success: false,
